...
const addRole = () => {
  ...
  inquirer
    ...
    .then((result) => {
      db.promise()
        .query(`SELECT id FROM department WHERE name = ?`, result.dept)
        .then((answer) => {
        ...
        .then((mappedId) => {
          db.promise().query(
            `INSERT INTO roles(title, salary, department_id)
                VALUES(?, ?, ?)`,
            [result.title, result.salary, mappedId]
          );
          init();
        });
    });
...
const addEmployee = () => {
  ...
  inquirer
    ...
    .then((result) => {
      ...
      db.promise()
        .query(`SELECT id FROM roles WHERE title = ?`, result.employeeRole)
        .then((answer) => {
        ...
        .then((roleId) => {
          db.query(
            `INSERT INTO employees(first_name, last_name, role_id)
                    VALUES(?, ?, ?)`,
            [result.firstName, result.lastName, roleId],
            (err, results) => {
              if (err) {
                console.log(err);
              } else {
